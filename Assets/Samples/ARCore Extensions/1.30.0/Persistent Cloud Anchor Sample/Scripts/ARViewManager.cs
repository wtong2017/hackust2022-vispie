...
namespace Google.XR.ARCoreExtensions.Samples.PersistentCloudAnchors
{
    ...
    public class ARViewManager : MonoBehaviour
    {
        ...
        public List<Anchor> contentAnchors = new List<Anchor>();
        ...
        public void Update() {
            ...
            // TODO: keep update the cached cloud anchor
            UpdateCacheAnchors();
        }

        private void UpdateCacheAnchors() {
            foreach (var cloudAnchor in _cachedCloudAnchors) {
                foreach (var item in contentAnchors) {
                    if (item.anchorID == cloudAnchor.cloudAnchorId) {
                        //Debug.Log($"state: {cloudAnchor.trackingState}");
                        if (cloudAnchor.trackingState == TrackingState.Tracking) {
                            var targetObject = GameObject.Find(item.nameInGame);
                            //Debug.Log(origin.transform.position);
                            //Debug.Log(cloudAnchor.transform.position);
                            //Debug.Log(cloudAnchor.pose.position);
                            //Debug.Log(Camera.main.transform.position);
                            //Debug.Log(targetObject.transform.position);
                            //Debug.Log(cloudAnchor.transform.position + Camera.main.transform.position);
                            //origin.transform.position = cloudAnchor.transform.position;
                            //origin.transform.rotation = cloudAnchor.transform.rotation;
                            if (Vector3.Distance(targetObject.transform.position, cloudAnchor.transform.position) > 0.01f) { // 1cm different
                                origin.MakeContentAppearAt(targetObject.transform, cloudAnchor.transform.position, cloudAnchor.transform.localRotation);

                            }
                            //Debug.Log(Camera.main.transform.position);
                            //origin.transform.position += Camera.main.transform.position;
                            //Debug.Log(origin.transform.position);
                            //StartCoroutine(Wait(item, cloudAnchor));
                        }
                    }
            ...
            foreach (var cloudAnchor in _pendingCloudAnchors) {
                if (cloudAnchor.cloudAnchorState == CloudAnchorState.Success) {
                    if (Controller.Mode == PersistentCloudAnchorsController.ApplicationMode.Hosting) {
                        ...
                        // check anchor names to anchor different points
                        //foreach (var item in contentAnchors) {
                        //    if (item.anchorID == cloudAnchor.cloudAnchorId) {
                        //        var targetObject = GameObject.Find(item.nameInGame);
                        //        //Debug.Log(origin.transform.position);
                        ...
                        //        //origin.transform.position = cloudAnchor.transform.position;
                        //        //origin.transform.rotation = cloudAnchor.transform.rotation;
                        //        //origin.MakeContentAppearAt(targetObject.transform, cloudAnchor.pose.position, cloudAnchor.pose.rotation);
                        //        //Debug.Log(Camera.main.transform.position);
                        //        //origin.transform.position += Camera.main.transform.position;
                        //        //Debug.Log(origin.transform.position);
                        //        //StartCoroutine(Wait(item, cloudAnchor));
        ...
        IEnumerator Wait(Anchor item, ARCloudAnchor cloudAnchor) {
